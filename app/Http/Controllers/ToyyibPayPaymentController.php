...
class ToyyibPayPaymentController extends Controller
{
    ...
    public function processPayment(Request $request)
    {
        ...
        try {
            ...
            // Get Superadmin settings for Plan Purchase
            $superadminSettings = getPaymentGatewaySettings();
            $payment_settings = $superadminSettings['payment_settings'];
            
            $secretKey = $payment_settings['toyyibpay_secret_key'] ?? '';
            $categoryCode = $payment_settings['toyyibpay_category_code'] ?? '';
            $mode = $payment_settings['toyyibpay_mode'] ?? 'sandbox';
            
            if (!$secretKey || !$categoryCode) {
                return back()->withErrors(['error' => __('ToyyibPay payment gateway not configured properly')]);
            }
            
            $url = ($mode == 'live') ? 'https://toyyibpay.com/index.php/api/createBill' : 'https://dev.toyyibpay.com/index.php/api/createBill';
            $redirectBaseUrl = ($mode == 'live') ? 'https://toyyibpay.com/' : 'https://dev.toyyibpay.com/';
            ...
            if ($curlError) {
                Log::error('ToyyibPay cURL Error: ' . $curlError);
                throw new \Exception('cURL Error: ' . $curlError);
            }
            
            if ($httpCode !== 200) {
                Log::error('ToyyibPay HTTP Error: ' . $httpCode, ['response' => $result]);
            ...
            if (isset($responseData[0]['BillCode'])) {
                $redirectUrl = $redirectBaseUrl . $responseData[0]['BillCode'];
            ...
            Log::error('ToyyibPay Payment Error: ' . $e->getMessage(), [
                'exception' => $e,
                'request' => $request->all()
            ]);
    ...
    public function callback(Request $request)
    {
        try {
            ...
            if ($status_id == '1') { // Payment successful
                if (str_starts_with($order_id, 'store_')) {
                    // Handle Store Order
                    $order = \App\Models\Order::where('payment_transaction_id', $billcode)
                        ->orWhere('payment_details->external_reference', $order_id)
                        ->first();
