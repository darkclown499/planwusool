...
class CourierRegistry
{
     ...
     */
    public static function catalog(bool $forMerchant = false): array
    {
        $all = [
            // === Palestinian — aggregator platform ===
            ['slug'=>'togo','name'=>'TOGO','name_ar'=>'TOGO','region'=>'local','type'=>'aggregator','status'=>'manual','coverage'=>'الضفة • القدس • غزة','services'=>['عدة شركات','تتبع','تحصيل ودفع'],'site'=>'https://api.togo.ps','logo'=>'/images/couriers/togo-remote.png','evidence'=>'Verified: https://api.togo.ps/ar + en — TOGO connects merchants with delivery companies and payment gateways, escrow; app Togo Merchant; aggregator platform, not single courier — partnership required'],
            ...
            // === Global — READY API ===
            ['slug'=>'aramex','name'=>'Aramex','name_ar'=>'أرامكس','region'=>'global','type'=>'courier','status'=>'ready','coverage'=>'محلي ودولي','services'=>['إنشاء الشحنات','تتبع','بوليصة شحن','تحديث تلقائي','تحصيل عند التسليم'],'site'=>'https://www.aramex.com/developers','logo'=>'/images/couriers/aramex-official2.webp','evidence'=>'VERIFIED API: aramex.com/developers — Shipping Services API'],
            ...
            ['slug'=>'fedex','name'=>'FedEx','name_ar'=>'فيديكس','region'=>'global','type'=>'courier','status'=>'ready','coverage'=>'دولي','services'=>['إنشاء الشحنات','تتبع','بوليصة شحن','تحصيل عند التسليم'],'site'=>'https://developer.fedex.com','logo'=>'/images/couriers/fedex-remote.svg','evidence'=>'VERIFIED API: FedEx Developer Portal — Ship & Track'],
            ['slug'=>'ups','name'=>'UPS','name_ar'=>'يو بي إس','region'=>'global','type'=>'courier','status'=>'ready','coverage'=>'دولي','services'=>['إنشاء الشحنات','تتبع','بوليصة شحن'],'site'=>'https://developer.ups.com','logo'=>'/images/couriers/ups-remote.svg','evidence'=>'VERIFIED API: UPS Developer Portal'],
        ...
        if ($forMerchant && app()->environment('production')) {
            $all = array_values(array_filter($all, fn($c)=>$c['slug']!=='mock'));
        }
        return $all;
    }

    public static function merchantCatalog(): array
    {
        return self::catalog(true);
    }
