...
import { Search, Plus, Minus, Trash2, ShoppingCart, CheckCircle2, X, Loader2, User as UserIcon, LogOut } from 'lucide-react';
...
export default function PosTerminalRegister() {
    const { t } = useTranslation();
    const page = usePage().props as any;
    const storeCurrency: string = page.storeCurrency || 'ILS';
    const storeName: string = page.storeName || '';
    const terminal: { id: number; name: string; username: string; store_id: number } = page.terminal || {};
    ...
    const [saleResult, setSaleResult] = useState<{ order_number: string } | null>(null);
    ...
    const loadCategories = () => {
        axios.get(route('pos.terminal.categories'))
            .then((res: { data: { success: boolean; categories?: { id: number; name: string }[] } }) => {
                if (res.data?.success) {
    ...
    const runSearch = (term: string, cat: string = categoryId) => {
        ...
        axios.get(route('pos.terminal.search'), { params: { q: term, category_id: cat || undefined, per_page: 20 } })
    ...
    // Cart-aware available stock: authoritative - in-cart same product/variant.
    ...
    const loadCustomers = (term: string) => {
        axios.get(route('pos.terminal.customers'), { params: { q: term, per_page: 20 } })
    ...
    const submitSale = () => {
        ...
        setSubmitting(true);
        setSaleError(null);
        axios.post(route('pos.terminal.sale'), {
            items: cart.map((l) => ({
            ...
            .then((res: { data: { order_number: string } }) => {
                setSaleResult({ order_number: res.data.order_number });
                setCart([]);
                setNotes('');
                setCustomerId(null);
    ...
    return (
        <>
            <Head title={t('Terminal POS')} />
            <div className="min-h-screen bg-muted/40">
                {/* Terminal header — dedicated cash-register chrome, NO merchant nav/settings. */}
                <header className="flex items-center justify-between border-b border-border bg-card px-4 py-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-bold leading-tight">{storeName || t('Store')}</p>
                            <p className="text-xs text-muted-foreground">
                                {terminal.name || ''} · {terminal.username || ''}
                            </p>
                        </div>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => router.post(route('pos.terminal.logout'))}>
