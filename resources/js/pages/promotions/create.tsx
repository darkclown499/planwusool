...
import { addDays, eachDayOfInterval, endOfMonth, format, startOfMonth, subMonths, addMonths } from 'date-fns';
...
export default function CreatePromotion({ products, categories }: Props) {
  const { t } = useTranslation();
  ...
  const [formData, setFormData] = useState({
    ...
    discount_value: '',
    max_discount_amount: '',
    quantity_tiers: [] as { min_qty: string; discount_value: string; max_discount_amount: string }[],
  ...
  const handleSubmit = () => {
    const submissionData = {
      ...
      quantity_tiers: formData.quantity_tiers.map((tier) => ({
    ...
    router.post(route('promotions.store'), submissionData);
  };
  ...
  return (
    <PageTemplate
      ...
      description={t('Create a promotional offer with smart discount rules')}
