...
export default function HeroComputerDemo({
  ...
  // keep latest muted value accessible inside callbacks
  ...
  /* ── Stage: desktop → mouse moves to browser icon ── */
  useEffect(() => {
    ...
    const t1 = window.setTimeout(() => {
      ...
      flyTo(browserIconRef.current, 1500);
    }, 800);
  ...
  const clockTime = now.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <div className="mx-auto w-full max-w-[620px]" dir="rtl">
      ...
      <div className="relative rounded-[22px] border border-slate-600/60 bg-gradient-to-b from-slate-700 via-slate-800 to-slate-900 p-2 shadow-[0_40px_90px_-24px_rgba(0,0,0,0.75)] sm:p-3">
        {/* soft brand glow behind the screen */}
        <div
        ...
        <div ref={screenRef} className="relative aspect-[16/10] overflow-hidden rounded-[12px] bg-black ring-1 ring-black/60">
          ...
          {stage === 'boot' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-[#06090b]">
              ...
              {appLogo ? (
                <img src={appLogo} alt={appName} className="boot-logo h-16 w-auto max-w-[75%] object-contain" />
              ) : (
                <span
          ...
          {showDesktopUI(stage) && (
            <>
              ...
              >
                <div
                  className="absolute -right-16 -top-20 h-64 w-64 rounded-full opacity-25 blur-3xl"
                  style={{ background: brandColor }}
                />
                <div
              ...
              {/* desktop icons */}
              {stage === 'desktop' && (
                <div className="absolute right-auto left-2 top-2 z-10 hidden flex-col gap-2 sm:flex">
                  <DesktopIcon ref={browserIconRef} icon={<Globe size={18} />} label="المتصفح" brandColor={brandColor} active />
                  ...
                  <DesktopIcon icon={<Camera size={18} />} label="الكاميرا" brandColor={brandColor} />
                </div>
              )}
              ...
              {showBrowser && (
                <div className="app-open absolute inset-1 bottom-[44px] z-10 flex flex-col overflow-hidden rounded-lg bg-white shadow-2xl">
                  ...
                  <div className="flex items-center gap-1.5 border-b border-gray-200 bg-gray-50 px-2 py-1">
                    ...
                    <RefreshCw
                      size={11}
                      className={stage === 'loading' ? 'animate-spin text-blue-500' : 'text-gray-500'}
                    />
                    <div
                      ref={addressRef}
              ...
              <div className="absolute inset-x-0 bottom-0 z-20 flex h-10 items-center justify-between border-t border-white/10 bg-white/[0.06] px-2 backdrop-blur">
                <div className="flex items-center gap-2">
                  <span
                    className="flex h-5 w-5 items-center justify-center rounded-md bg-white/10 border border-white/10"
                    onClick={waClick}
                  >
          ...
          {cursorVisible && (
            ...
            >
              <svg
                ...
                height="22"
                viewBox="0 0 17 22"
                className={`drop-shadow-md ${clicking ? 'cursor-press' : ''}`}
              >
                <path
          ...
          {/* product detail card */}
          {detail && (
            <div className="backdrop-fade absolute inset-0 z-50 flex items-end justify-center bg-black/55 sm:items-center sm:p-4" onClick={closeDetail}>
              <div
              ...
              >
                ...
                <div className="px-3 pb-3 pt-2">
                  ...
                  <div className="mt-2 flex items-baseline gap-1.5">
                    ...
                    {detail.oldPrice && (
                      <span className="text-[9px] text-gray-400 line-through">{detail.oldPrice}₪</span>
                    )}
                  </div>
      ...
      <style>{`
        ...
        @keyframes bootLogoIn {
          from { opacity: 0; transform: scale(0.72); filter: blur(10px); }
          to { opacity: 1; transform: scale(1); filter: blur(0); }
        }
        .boot-logo { animation: bootLogoIn 1.05s ease forwards; }
        ...
        .scanlines {
          ...
          animation: crtFlicker 0.11s infinite;
        }
        ...
        .shimmer {
          background: linear-gradient(90deg, #e8eaed 25%, #f6f7f9 50%, #e8eaed 75%);
...
function showDesktopUI(stage: Stage) {
  return stage === 'desktop' || stage === 'browser' || stage === 'loading' || stage === 'demo';
}

const DesktopIcon = React.forwardRef<HTMLButtonElement, { icon: React.ReactNode; label: string; brandColor: string; active?: boolean }>(
  function DesktopIcon({ icon, label, brandColor, active }, ref) {
    return (
      <button
        ref={ref}
        className={`flex w-14 flex-col items-center gap-1 rounded-lg p-1.5 transition ${
      ...
      >
        <span
          ...
          style={{ background: `linear-gradient(135deg, ${brandColor}55, ${brandColor}22)` }}
        >
          {icon}
        </span>
...
/* ── live storefront inside the browser ── */
interface StoreViewProps {
  ...
  cardARef: React.RefObject<HTMLDivElement | null>;
  cardBRef: React.RefObject<HTMLDivElement | null>;
}

function StoreView({ store, brandColor, onOpenDetail, onWaClick, waBtnRef, bubbleRef, cardARef, cardBRef }: StoreViewProps) {
  return (
    <div className="flex h-full flex-col bg-white" dir="rtl" style={{ fontFamily: "'Tajawal', 'Segoe UI', sans-serif" }}>
      ...
      <div className="flex items-center justify-between border-b border-gray-100 px-2 py-1.5">
        ...
        <div className="relative flex items-center">
          ...
          <span className="absolute -right-1.5 -top-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full text-[7px] font-bold text-white" style={{ background: brandColor }}>
            2
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-1.5 pb-3">
        <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-4">
          {store.products.map((product, i) => (
            <div
              key={product.name}
              ref={i === 0 ? cardARef : i === 1 ? cardBRef : undefined}
            ...
            >
              ...
              <div className="mt-0.5 flex items-baseline gap-1">
                ...
                {product.oldPrice && (
                  <span className="text-[6.5px] text-gray-400 line-through">{product.oldPrice}₪</span>
                )}
              </div>
              <button
