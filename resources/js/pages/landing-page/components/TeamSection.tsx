      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
            {t(sectionData?.title || 'Meet Our Wusool Team')}
          </h2>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed font-medium">
            {t(sectionData?.subtitle || 'We\'re a diverse team of innovators, designers, and problem-solvers united by our passion for transforming professional networking.')}
          </p>
        </div>
            ...
            <div key={index} className="bg-gray-50 rounded-xl p-6 border border-gray-200 hover:border-gray-300 transition-colors">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">
                  {t(member.name, { defaultValue: member.name })}
                </h3>
                <p className="text-gray-700 font-medium mb-3">
                  {t(member.role, { defaultValue: member.role })}
                </p>
                <p className="text-gray-600 text-sm leading-relaxed mb-4">
                  {t(member.bio, { defaultValue: member.bio })}
                </p>
          ...
          <div className="text-center mt-8 sm:mt-12 lg:mt-16">
            <div className="bg-gray-50 rounded-xl p-8 border border-gray-200 max-w-2xl mx-auto">
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {t(sectionData?.cta_title || 'Want to Join Our Team?')}
              </h3>
              <p className="text-gray-600 mb-6">
                {t(sectionData?.cta_description || 'We\'re always looking for talented individuals who share our passion for innovation and excellence. Check out our open positions.')}
              </p>
              <button className="text-white px-8 py-3 rounded-lg transition-colors font-semibold" style={{ backgroundColor: brandColor }}>
                {t(sectionData?.cta_button_text || 'View Open Positions')}
              </button>
            </div>
          </div>
        )}
