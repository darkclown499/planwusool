...
export default function StoreAppearance({ store, currentTemplate, userPlanName, userPlanTier = 'starter', isSuperAdmin = false }: Props) {
    ...
    const previewUrl = useMemo(() => `${store.store_url}?v=${reloadKey}`, [store.store_url, reloadKey]);
    ...
    return (
                ...
                return (
                    <Button asChild variant="outline">
                        <Link href={templateSelectHref}>
                            <LayoutTemplate className="me-1 h-4 w-4" />
                            {t('Change Template')}
                        </Link>
                    </Button>
                );
        ...
        >
            <div className="space-y-6">
                {/* Editor tabs */}
                <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white p-1">
                    <button
                        type="button"
                        onClick={() => setActiveTab('template')}
                        className={`inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeTab === 'template' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                    ...
                    <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeTab === 'content' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                ...
                <Card>
                    <CardContent className="flex flex-wrap items-center justify-between gap-3 py-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-gray-900">{currentTemplate ? currentTemplate.name : store.template_slug}</p>
                                <p className="text-sm text-gray-500">{currentTemplate?.description || t('No template configured yet.')}</p>
                            </div>
                        </div>
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    {activeTab === 'template' ? (
                        ...
                        /* Content & banners editor */
                        <Card>
                            <CardHeader>
                                <CardTitle>{t('Content & Banners')}</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <StoreContentEditor
                                    storeId={store.id}
