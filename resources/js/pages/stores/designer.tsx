        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
            ...
            <div className="mt-2">
                <div className="flex items-center justify-between">
                    ...
                    <span className={`rounded-full px-2 py-0.5 font-mono text-[10px] font-bold ring-1 ${safeFit==='contain' ? 'bg-slate-100 text-slate-400 ring-slate-200' : 'bg-white text-slate-700 ring-slate-200'}`}>{safeFit==='contain' ? '100%' : `${Math.round(safeZoom*100)}%`}</span>
                </div>
                {safeFit === 'contain' ? (
                    <p className="mt-1 rounded-lg bg-amber-50 px-2 py-1.5 text-[10px] font-bold text-amber-700 ring-1 ring-amber-200">التكبير متاح في وضع تعبئة البنر</p>
                ) : (
                    ...
                    <div className="flex-1 overflow-y-auto px-3 py-3 sm:px-3 sm:py-3 lg:max-h-[calc(100vh-56px-118px)] [scrollbar-width:thin] [scrollbar-color:theme(colors.slate.200)_transparent] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-track]:bg-transparent">
                                        ...
                                        {displayImages.map((item:any, idx:number)=>{
                                            ...
                                            <div key={item.id||idx} className="rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm">
                                                ...
                                                <HeroCropEditor
                                                    ...
                                                    onFitChange={(f) => {
                                                        if (hasMedia) {
                                                            if (f === 'contain') updateHeroMedia(String(item.id), { fit: f, zoom: 1 });
                                                            else updateHeroMedia(String(item.id), { fit: f });
                                                        } else {
                                                ...
                                                <HeroCropEditor
                                                    ...
                                                    onFitChange={(f) => {
                                                        if (hasMedia) {
                                                            if (f === 'contain') updateHeroMedia(String(item.id), { fitMobile: f, zoomMobile: 1 });
                                                            else updateHeroMedia(String(item.id), { fitMobile: f });
                                                        } else {
                                        ...
                                        {allVideos.length===0 ? <p className="mt-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-3 py-3 text-center text-xs text-slate-500">لا يوجد فيديو — فارغ</p> : allVideos.map((v:any, idx:number)=>{
                                          ...
                                          <div key={v.id||idx} className="group relative mt-3 rounded-xl border bg-white p-2.5">
                                            ...
                                            <HeroCropEditor
                                              ...
                                              onFitChange={(f)=> { if (String(v.id)==='legacy-video') { const hyd=[{id:`video-${Date.now().toString(36)}`,type:'video',src:v.src,fit:f,position:'50% 50%',positionMobile:'50% 50%',zoom:1}]; let tmp=setDotted(content,'hero_banner.media',hyd); tmp=setDotted(tmp,'hero_banner.video_url',v.src); tmp=setDotted(tmp,'hero_video_url',v.src); setContent(tmp);} else { if (f==='contain') updateHeroMedia(String(v.id),{fit:f, zoom:1}); else updateHeroMedia(String(v.id),{fit:f}); } }}
                                            ...
                                            <HeroCropEditor
                                              ...
                                              onFitChange={(f)=> { if (String(v.id)==='legacy-video') { const hyd=[{id:`video-${Date.now().toString(36)}`,type:'video',src:v.src,position:'50% 50%',positionMobile:'50% 50%',fitMobile:f,zoomMobile:1}]; let tmp=setDotted(content,'hero_banner.media',hyd); setContent(tmp);} else { if (f==='contain') updateHeroMedia(String(v.id),{fitMobile:f, zoomMobile:1}); else updateHeroMedia(String(v.id),{fitMobile:f}); } }}
                        ...
                        {activeWorkspace === 'content' && (
                            <div className="space-y-4">
                                ...
                                {theme === 'electronics-hub' && (
                                    <Card>
                                        <p className="mb-2 text-xs font-black text-slate-800">تقنية — زر واتساب العائم (جوال) <span className="ms-1 rounded bg-green-50 px-1.5 py-0.5 text-[10px] font-bold text-green-700">WhatsApp</span></p>
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                                                <span className="text-xs font-bold text-slate-700">تفعيل زر واتساب العائم</span>
                                                <Switch checked={!!getDotted(content,'electronics_whatsapp.enabled')} onCheckedChange={(v)=> setContent(setDotted(content,'electronics_whatsapp.enabled', v))} />
                                            </div>
                                            <div>
                                                <SectionLabel hint="يُستخدم إن وجد، وإلا رقم واتساب العام للمتجر">رقم واتساب (مع رمز البلد)</SectionLabel>
                                                <Input dir="ltr" value={String(getDotted(content,'electronics_whatsapp.number') ?? '')} onChange={(e)=> setContent(setDotted(content,'electronics_whatsapp.number', e.target.value.replace(/[^0-9+]/g,'')))} placeholder="9705XXXXXXXX" className="bg-white text-sm" />
                                            </div>
                                            <div>
                                                <SectionLabel>الرسالة الافتراضية</SectionLabel>
                                                <Textarea value={String(getDotted(content,'electronics_whatsapp.message') ?? '')} onChange={(e)=> setContent(setDotted(content,'electronics_whatsapp.message', e.target.value))} placeholder="مرحباً، لدي استفسار عن أحد المنتجات" rows={2} className="bg-white" />
                                                <p className="mt-1 text-[11px] text-slate-400">يتم ترميزها تلقائياً عبر wa.me</p>
                                            </div>
                                        </div>
                                    </Card>
                                )}
                                {theme === 'electronics-hub' && (
                                    <Card>
                                        <p className="mb-2 text-xs font-black text-slate-800">تقنية — روابط التواصل في القائمة (6 خانات) <span className="ms-1 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-600">هامبرغر</span></p>
                                        <p className="mb-3 text-[11px] leading-relaxed text-slate-500">تظهر داخل قائمة الهامبرغر للجوال فقط قرب الأسفل. كل خانة قابلة للتفعيل/التعطيل مع اختيار المنصة والرابط.</p>
                                        <div className="space-y-4">
                                            {[1,2,3,4,5,6].map((idx) => {
                                                const enabled = !!getDotted(content, `electronics_mobile_nav.social_${idx}_enabled`);
                                                const platform = String(getDotted(content, `electronics_mobile_nav.social_${idx}_platform`) ?? 'instagram');
                                                const url = String(getDotted(content, `electronics_mobile_nav.social_${idx}_url`) ?? '');
                                                return (
                                                    <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                                        <div className="mb-2 flex items-center justify-between">
                                                            <span className="text-xs font-black text-slate-700">رابط {idx}</span>
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-[11px] font-bold text-slate-500">مفعل</span>
                                                                <Switch checked={enabled} onCheckedChange={(v)=> setContent(setDotted(content, `electronics_mobile_nav.social_${idx}_enabled`, v))} />
                                                            </div>
                                                        </div>
                                                        <div className="grid grid-cols-2 gap-2">
                                                            <div>
                                                                <SectionLabel>المنصة</SectionLabel>
                                                                <select value={platform} onChange={(e)=> setContent(setDotted(content, `electronics_mobile_nav.social_${idx}_platform`, e.target.value))} className="w-full rounded-xl border border-slate-200 bg-white px-2 py-2 text-xs focus:border-[#2563eb] focus:outline-none">
                                                                    <option value="facebook">Facebook</option>
                                                                    <option value="instagram">Instagram</option>
                                                                    <option value="tiktok">TikTok</option>
                                                                    <option value="youtube">YouTube</option>
                                                            ...
                                                            <div>
                                                                <SectionLabel>الرابط</SectionLabel>
                                                                <Input dir="ltr" value={url} onChange={(e)=> setContent(setDotted(content, `electronics_mobile_nav.social_${idx}_url`, e.target.value.trim()))} placeholder="https://..." className="bg-white text-xs" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                );
                                ...
                                {theme === 'bazaar-market' && (
                                    <Card>
                                        <p className="mb-2 text-xs font-black text-slate-800">بازار — زر واتساب العائم (جوال) <span className="ms-1 rounded bg-green-50 px-1.5 py-0.5 text-[10px] font-bold text-green-700">WhatsApp</span></p>
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                                                <span className="text-xs font-bold text-slate-700">تفعيل زر واتساب العائم</span>
                                                <Switch checked={!!getDotted(content,'bazaar_whatsapp.enabled')} onCheckedChange={(v)=> setContent(setDotted(content,'bazaar_whatsapp.enabled', v))} />
                                            </div>
                                            <div>
                                                <SectionLabel hint="يُستخدم إن وجد، وإلا رقم واتساب العام للمتجر">رقم واتساب (مع رمز البلد)</SectionLabel>
                                                <Input dir="ltr" value={String(getDotted(content,'bazaar_whatsapp.number') ?? '')} onChange={(e)=> setContent(setDotted(content,'bazaar_whatsapp.number', e.target.value.replace(/[^0-9+]/g,'')))} placeholder="9665XXXXXXXX" className="bg-white text-sm" />
                                            </div>
                                            <div>
                                                <SectionLabel>الرسالة الافتراضية</SectionLabel>
                                                <Textarea value={String(getDotted(content,'bazaar_whatsapp.message') ?? '')} onChange={(e)=> setContent(setDotted(content,'bazaar_whatsapp.message', e.target.value))} placeholder="مرحباً، لدي استفسار بخصوص المتجر" rows={2} className="bg-white" />
                                                <p className="mt-1 text-[11px] text-slate-400">يتم ترميزها تلقائياً عبر wa.me</p>
                                            </div>
                                        </div>
                                    </Card>
                                ...
                                {theme === 'bazaar-market' && (
                                    <Card>
                                        <p className="mb-2 text-xs font-black text-slate-800">بازار — روابط التواصل في القائمة (6 خانات) <span className="ms-1 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-600">هامبرغر</span></p>
                                        <p className="mb-3 text-[11px] leading-relaxed text-slate-500">تظهر داخل قائمة الهامبرغر للجوال فقط قرب الأسفل. كل خانة قابلة للتفعيل/التعطيل مع اختيار المنصة والرابط.</p>
                                        <div className="space-y-4">
                                            {[1,2,3,4,5,6].map((idx) => {
                                                const enabled = !!getDotted(content, `bazaar_mobile_nav.social_${idx}_enabled`);
                                                const platform = String(getDotted(content, `bazaar_mobile_nav.social_${idx}_platform`) ?? 'instagram');
                                                const url = String(getDotted(content, `bazaar_mobile_nav.social_${idx}_url`) ?? '');
                                                return (
                                                    <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                                        <div className="mb-2 flex items-center justify-between">
                                                            <span className="text-xs font-black text-slate-700">رابط {idx}</span>
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-[11px] font-bold text-slate-500">مفعل</span>
                                                                <Switch checked={enabled} onCheckedChange={(v)=> setContent(setDotted(content, `bazaar_mobile_nav.social_${idx}_enabled`, v))} />
                                                            </div>
                                                        </div>
                                                        <div className="grid grid-cols-2 gap-2">
                                                            <div>
                                                                <SectionLabel>المنصة</SectionLabel>
                                                                <select value={platform} onChange={(e)=> setContent(setDotted(content, `bazaar_mobile_nav.social_${idx}_platform`, e.target.value))} className="w-full rounded-xl border border-slate-200 bg-white px-2 py-2 text-xs focus:border-teal-600 focus:outline-none">
                                                                    <option value="facebook">Facebook</option>
                                                                    <option value="instagram">Instagram</option>
                                                                    <option value="tiktok">TikTok</option>
                                                                    <option value="youtube">YouTube</option>
                                                            ...
                                                            <div>
                                                                <SectionLabel>الرابط</SectionLabel>
                                                                <Input dir="ltr" value={url} onChange={(e)=> setContent(setDotted(content, `bazaar_mobile_nav.social_${idx}_url`, e.target.value.trim()))} placeholder="https://..." className="bg-white text-xs" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                );
