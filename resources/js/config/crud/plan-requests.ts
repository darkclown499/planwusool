  ...
  table: {
    ...
    actions: [
      { 
        ...
        condition: (item: any, permissions?: string[]) => item.status === 'pending' && (permissions?.includes('approve-plan-requests') ?? true)
      },
      { 
        label: t('Reject'), 
        icon: 'X', 
        ...
        condition: (item: any, permissions?: string[]) => item.status === 'pending' && (permissions?.includes('reject-plan-requests') ?? true)
      }
    ]
  },
  search: {
