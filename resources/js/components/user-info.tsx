    ...
    return (
        <div dir={position === 'right' ? 'rtl' : position === 'left' ? 'ltr' : undefined} className="flex w-full min-w-0 items-center gap-2 overflow-hidden">
            ...
            <div className={`grid flex-1 min-w-0 text-sm leading-tight text-start overflow-hidden ${className}`}>
                <span className="truncate font-medium leading-none text-[12.5px] text-gray-800">{user.name}</span>
                {showEmail ? (
                    <span className="text-muted-foreground truncate text-[11px] leading-none">{user.email}</span>
                ) : (
                    <span className="truncate text-[11px] leading-none text-gray-500">{user.email}</span>
                )}
            </div>
        </div>
    );
