...
export function MidtransPaymentForm({
  ...
  const initializeMidtransSnap = (snapToken: string, orderId: string) => {
    // Inject global style to fix Radix UI Dialog pointer-events lock
    if (!document.getElementById('midtrans-pointer-fix')) {
      const style = document.createElement('style');
      style.id = 'midtrans-pointer-fix';
      style.innerHTML = `
        body { pointer-events: auto !important; }
        #snap-midtrans { pointer-events: auto !important; z-index: 999999 !important; }
      `;
  ...
  const removeMidtransFix = () => {
    const style = document.getElementById('midtrans-pointer-fix');
