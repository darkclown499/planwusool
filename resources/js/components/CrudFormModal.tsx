      ...
      <DialogContent className={`${getModalSizeClass()} max-h-[90vh]`} modalId={modalId}>
        ...
        <ScrollArea className="max-h-[70vh] pe-4">
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            {layout === 'grid' ? (
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${formConfig.columns || 1}, 1fr)`, gap: '1rem' }}>
                {formConfig.fields.map((field) => {
                  ...
                  return (
                    ...
                    >
                      {field.type !== 'checkbox' && (
                        <Label htmlFor={field.name} className="text-sm font-medium" required={field.required && !(field.type === 'file' && mode === 'edit')}>
                          {field.label}
                        </Label>
                      )}
                      {renderField(field)}
