...
export const AtelierCartDrawer: React.FC<AtelierCartDrawerProps> = ({ onClose, onCheckout, onProductClick }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60]" dir="rtl" role="dialog" aria-modal="true">
      ...
      <aside className="absolute inset-y-0 end-0 flex w-full max-w-md flex-col bg-[#faf7f2] shadow-2xl pb-[env(safe-area-inset-bottom)]" style={{ animation: 'atelierSlideRight .35s cubic-bezier(.22,.9,.3,1)' }}>
        {/* Head */}
      ...
      <style>{`@keyframes atelierSlideRight{from{transform:translateX(100%)}to{transform:translateX(0)}}`}</style>
    </div>
  );
};
