...
export function SouqFooter() {
  ...
  return (
    <footer className="mt-10 bg-[#14532d] pb-20 text-stone-300 sm:pb-0" dir="rtl">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:grid-cols-3 sm:px-6 lg:px-8">
        <div>
          <p className="mb-3 flex items-center gap-2 text-lg font-black text-white">
            <ShoppingBasket className="h-5 w-5 text-[#fbbf24]" /> {config?.storeName || store?.name}
          </p>
          <p className="text-sm leading-relaxed text-stone-400">
            خضار وفواكه ومواد غذائية طازجة تصلك حتى باب البيت — جودة مختارة بعناية كل صباح.
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-stone-400">
            {!!config?.phoneNumber && <li dir="ltr" className="text-right">{config.phoneNumber}</li>}
            {!!config?.address && <li>{config.address}</li>}
            <li>يومياً: 8 صباحاً — 11 مساءً</li>
          </ul>
        </div>
      </div>
      <div className="border-t border-white/10 py-4 text-center text-xs text-stone-500">
        © {new Date().getFullYear()} {config?.storeName || store?.name} — أسعار الجملة للجميع
      </div>
    </footer>
  );
}
