        ...
        <div className="flex-1 overflow-y-auto p-4 px-6 bg-gray-50">
          {cartItems.length === 0 ? (
            <div className="text-center mt-20">
              ...
              <button
                onClick={onClose}
                className="bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-lg font-medium transition-colors cursor-pointer"
              >
                متابعة التسوق
              </button>
            </div>
                ...
                <div key={index} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 
                        onClick={() => onProductClick(item)}
                        className="font-semibold text-gray-900 mb-1 text-sm leading-tight cursor-pointer hover:text-orange-600 transition-colors"
                      >
                        {item.name}
                      </h3>
                      {(() => {
                        ...
                        <div className="flex items-center gap-3 bg-gray-100 rounded-lg p-1">
                          <button 
                            ...
                            className={`w-7 h-7 flex items-center justify-center rounded-md text-white text-sm shadow-sm font-bold leading-none ${
                              item.quantity > 1 ? 'bg-orange-600 hover:bg-orange-700 cursor-pointer' : 'bg-gray-400 cursor-not-allowed'
                            }`}
                          >
                            −
                          </button>
            ...
            <div className="bg-gray-50 rounded-2xl p-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <span className="text-2xl font-bold text-orange-600">{formatCurrency(total, storeSettings, currencies)}</span>
                </div>
              </div>
            </div>
            <button 
              onClick={onCheckout}
              className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-4 px-6 rounded-2xl transition-colors shadow-lg hover:shadow-xl cursor-pointer flex items-center justify-center gap-2 mt-4"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l-1 12H6L5 9z" />
              </svg>
