      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-lg bg-white shadow-2xl flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-emerald-100 text-sm">{cartItems.length} منتجات مختارة بعناية</p>
              </div>
            </div>
            <button onClick={onClose} className="w-10 h-10 flex items-center justify-center text-emerald-100 hover:text-white hover:bg-emerald-500 rounded-lg transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50">
          {cartItems.length === 0 ? (
            ...
            <div className="p-4">
              {/* Table Header */}
              <div className="bg-white rounded-t-lg border-b-2 border-emerald-200 p-3 mb-2">
              ...
              <div className="space-y-1 overflow-x-auto">
                {cartItems.map((item, index) => (
                  <div key={index} className="min-w-md bg-white rounded-lg p-3 border border-gray-200 hover:border-emerald-300 transition-colors">
                      ...
                      <div className="col-span-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <h3 
                              onClick={() => onProductClick(item)}
                              className="font-semibold text-gray-900 text-sm leading-tight cursor-pointer hover:text-emerald-600 transition-colors line-clamp-1"
                            >
                              {item.name}
                            </h3>
                              ...
                              return variants && Object.keys(variants).length > 0 && (
                                <div className="text-xs text-emerald-600 mt-1">
                                  {Object.entries(variants).map(([key, value], index) => (
                                    <span key={key} className="bg-emerald-100 px-1 py-0.5 rounded ml-1">
                                      {key}: {value}
                                    </span>
                                  ))}
                                </div>
                        ...
                        <div className="flex items-center justify-center gap-1">
                          <button 
                            ...
                            className={`w-6 h-6 flex items-center justify-center rounded text-xs font-bold ${
                              item.quantity > 1 ? 'bg-emerald-600 text-white hover:bg-emerald-700 cursor-pointer' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                            } transition-colors`}
                          >
                          ...
                          <button 
                            ...
                            className="w-6 h-6 flex items-center justify-center bg-emerald-600 hover:bg-emerald-700 rounded text-white text-xs font-bold cursor-pointer transition-colors"
                          >
        ...
        {cartItems.length > 0 && (
          <div className="bg-white border-t-2 border-emerald-200 p-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4 border-2 border-dashed border-gray-300">
              ...
              <div className="space-y-2 font-mono text-sm">
                ...
                <div className="border-t border-gray-400 pt-2 flex justify-between text-lg">
                  ...
                  <span className="font-bold text-emerald-600">{formatCurrency(total, storeSettings, currencies)}</span>
                </div>
              </div>
            </div>
            ...
            <button 
              onClick={onCheckout}
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 px-6 rounded-lg transition-colors shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
            >
