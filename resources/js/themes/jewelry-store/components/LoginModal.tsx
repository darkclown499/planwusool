    ...
    return (
      <div className="fixed inset-0 z-50 overflow-hidden" onClick={onClose}>
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col border border-yellow-100" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-yellow-100 bg-yellow-50 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-yellow-900">استعادة كلمة المرور</h2>
              </div>
              <button onClick={onClose} className="p-2 text-yellow-600 hover:text-yellow-800 hover:bg-yellow-100 rounded-lg transition-colors">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <p className="text-center text-sm text-yellow-700">
                  ...
                  <button 
                    ...
                    type="button" 
                    className="text-yellow-600 hover:text-yellow-800 font-medium mr-1"
                  >
                    تسجيل الدخول
                  </button>
                </p>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col border border-yellow-100" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between p-6 border-b border-yellow-100 bg-yellow-50 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-yellow-900">{isLogin ? 'تسجيل الدخول' : 'إنشاء حساب'}</h2>
            </div>
            <button onClick={onClose} className="p-2 text-yellow-600 hover:text-yellow-800 hover:bg-yellow-100 rounded-lg transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          ...
          <div className="p-6 overflow-y-auto">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isLogin && (
                <div>
                  <label className="block text-sm font-medium text-yellow-800 mb-2">رقم الهاتف</label>
                  <input 
                    type="tel" 
                    value={phone}
                  ...
                  />
                  <p className="text-xs text-yellow-600 mt-1">يرجى استخدام رمز الدولة (مثال: 970+)</p>
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-yellow-800 mb-2">كلمة المرور</label>
                <input 
                  type="password" 
                  value={password}
              ...
              {!isLogin && (
                <div>
                  <label className="block text-sm font-medium text-yellow-800 mb-2">تأكيد كلمة المرور</label>
                  <input 
                    type="password" 
