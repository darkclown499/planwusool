  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" onClick={onClose}>
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl md:rounded-3xl shadow-2xl w-full max-w-6xl max-h-[85vh] md:max-h-[70vh] overflow-hidden flex flex-col border border-yellow-100" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex flex-col md:flex-row flex-1 max-h-[calc(85vh-80px)] md:max-h-[calc(70vh-80px)] overflow-y-auto md:overflow-hidden">
            {/* Image Gallery */}
            <div className="w-full md:w-2/5 p-4 md:p-6 bg-yellow-50/50 md:overflow-hidden">
              <div className="h-64 md:h-full flex flex-col space-y-3 md:space-y-4">
                <div className="relative flex-1 bg-white rounded-2xl overflow-hidden shadow-xl border-2 border-yellow-200/50">
                  <img
                  ...
                  {product.images && product.images.length > 1 && (
                    <>
                      <button 
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 bg-yellow-600/90 hover:bg-yellow-700 text-white p-2.5 rounded-full shadow-lg transition-all hover:scale-110 rtl-flip"
                      >
                      ...
                      <button 
                        ...
                        className="absolute left-3 top-1/2 -translate-y-1/2 bg-yellow-600/90 hover:bg-yellow-700 text-white p-2.5 rounded-full shadow-lg transition-all hover:scale-110 rtl-flip"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        </svg>
                      </button>
                      <div className="absolute bottom-4 left-4 bg-yellow-900/80 text-white px-3 py-1 text-sm rounded-full">
                      ...
                      <button
                        ...
                        className={`flex-shrink-0 w-14 h-14 md:w-18 md:h-18 overflow-hidden border-2 rounded-lg md:rounded-xl transition-all ${
                          selectedImageIndex === index 
                            ? 'border-yellow-600 ring-2 ring-yellow-300 shadow-lg transform scale-105' 
            ...
            <div className="flex-1 flex flex-col md:overflow-hidden">
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
                ...
                <div className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-yellow-100">
                  <div className="space-y-3 md:space-y-4">
                    ...
                    <h1 className="text-2xl md:text-3xl font-serif font-bold text-yellow-900 leading-tight">{product.name}</h1>
                    
                    <div className="flex items-center justify-between pt-2 border-t border-yellow-100">
                      <div className="inline-flex items-center bg-yellow-50 px-3 py-1.5 rounded-full">
                        <span className="text-yellow-700 text-sm font-medium">الرمز: {product.sku}</span>
                      </div>
                ...
                {/* Pricing */}
                <div className="bg-yellow-50 rounded-2xl p-4 md:p-6 border border-yellow-200 shadow-sm">
                  <div className="text-center mb-4 md:mb-6">
                    <div className="text-3xl md:text-4xl font-bold text-yellow-700 mb-2">
                      {formatCurrency(product.price, storeSettings, currencies)}
                    </div>
                    {product.originalPrice && (
                      <div className="flex items-center justify-center gap-3">
                        <span className="text-xl text-yellow-400 line-through">
                  ...
                  <div className="flex items-center justify-center gap-6 bg-white rounded-xl p-4 border border-yellow-100">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="w-px h-4 bg-yellow-200"></div>
                    <span className="text-yellow-700 font-medium">متبقي {product.stockQuantity} وحدة</span>
                  </div>
                </div>
                ...
                {!(product.variants && Array.isArray(product.variants) && product.variants.length > 0) && (
                  <div className="bg-white rounded-2xl p-4 shadow-sm border border-yellow-100">
                    <button
                      ...
                      className="w-full bg-yellow-600 hover:bg-yellow-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] flex items-center justify-center gap-3"
                    >
                ...
                {/* Variants */}
                {product.variants && Array.isArray(product.variants) && product.variants.length > 0 && (
                  <div className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-yellow-100 space-y-4">
                    <h3 className="text-xl font-serif font-bold text-yellow-900 flex items-center gap-2">
                    ...
                    <div className="grid gap-4">
                      {product.variants.map((variant, index) => (
                        <div key={index}>
                          <label className="block text-sm font-semibold text-yellow-800 mb-2 capitalize">
                            {variant?.name || 'خيار'}
                          </label>
                          <Select 
                          ...
                          >
                            <SelectTrigger className="w-full focus:ring-yellow-500 focus:border-yellow-500 border-yellow-200 bg-yellow-50/30">
                ...
                {product.description && (
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-yellow-100">
                    <h3 className="text-xl font-serif font-bold text-yellow-900 mb-4 flex items-center gap-2">
                      <div className="w-1 h-6 bg-yellow-500 rounded-full"></div>
                      الوصف
                    </h3>
                    <div 
                      className="text-yellow-800 leading-relaxed prose prose-amber max-w-none"
                    ...
                    <div className="bg-yellow-50 rounded-xl p-3 md:p-4 border border-yellow-200">
                      <div className="space-y-2 md:space-y-3">
                        {product.customFields.map((field, index) => (
                          <div key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 px-3 bg-white rounded-lg border border-yellow-100 space-y-1 sm:space-y-0">
                            <span className="font-semibold text-yellow-800 text-sm md:text-base">{field.name}</span>
                            <span className="text-yellow-700 font-medium text-sm md:text-base break-words">{field.value}</span>
                          </div>
                        ))}
                      </div>
                    </div>
