      ...
      <div className="absolute inset-0 flex items-center justify-center p-2 md:p-4">
        <div className="bg-white rounded-xl md:rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="p-4 md:p-8 text-center">
            {/* Success Icon */}
            <div className="w-16 h-16 md:w-20 md:h-20 bg-sky-100 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
            ...
            {/* Order Invoice Link */}
            {orderNumber && (
              <div className="bg-sky-50 rounded-lg p-3 md:p-4 mb-4 md:mb-6">
                <p className="text-xs md:text-sm text-sky-600 mb-2">رابط فاتورة الطلب</p>
                <div className="flex flex-col md:flex-row gap-2">
                  <input 
                    type="text" 
                    value={orderLink}
                    readOnly
                    className="flex-1 text-xs md:text-sm bg-white border border-sky-200 rounded px-2 md:px-3 py-2 text-gray-700 select-all"
                  />
                  <button
                    onClick={copyOrderLink}
                    className="w-full md:w-auto bg-sky-600 hover:bg-sky-700 text-white px-3 py-2 rounded text-xs md:text-sm font-medium transition-colors cursor-pointer whitespace-nowrap"
                  >
                    نسخ الرابط
                  </button>
                </div>
            ...
            <div className="space-y-2 md:space-y-3">
              <button
                ...
                className="w-full bg-sky-600 hover:bg-sky-700 text-white font-semibold py-2.5 md:py-3 px-4 md:px-6 rounded-lg md:rounded-xl text-sm md:text-base transition-colors cursor-pointer"
              >
                متابعة التسوق
              </button>
              {orderNumber && (
