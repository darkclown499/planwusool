  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70" onClick={onClose}>
      <div className="flex items-center justify-center h-full p-2 sm:p-4">
        <div 
          className="bg-slate-800 w-full max-w-sm sm:max-w-md overflow-hidden shadow-2xl border-2 border-amber-600"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="relative p-4 sm:p-8 text-center bg-black border-b-2 border-amber-600">
            <button 
              onClick={onClose}
              className="absolute top-2 left-2 sm:top-4 sm:left-4 p-2 text-slate-400 hover:text-amber-400 hover:bg-slate-700 transition-colors"
            >
              <X className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-amber-600 flex items-center justify-center mx-auto mb-3 sm:mb-4">
          ...
          <div className="p-4 sm:p-8 space-y-4 sm:space-y-6">
            ...
            <button
              onClick={onLogin}
              className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 sm:py-4 px-4 sm:px-6 transition-all border-2 border-amber-600 hover:border-amber-500 flex items-center justify-center gap-2 sm:gap-3 group text-sm sm:text-base"
            >
