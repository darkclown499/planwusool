...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-sm sm:max-w-md bg-slate-800 shadow-2xl flex flex-col border-l-2 border-amber-600">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 border-amber-600 bg-black">
        ...
        <div className="flex-1 overflow-y-auto bg-slate-800">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              <div className="w-20 h-20 bg-amber-600 flex items-center justify-center mb-4">
              ...
              <button
                onClick={onClose}
                className="bg-amber-600 hover:bg-amber-700 text-white px-6 py-3 font-bold transition-colors border-2 border-amber-600"
              >
                متابعة التسوق
              </button>
            </div>
            ...
            <div className="p-3 sm:p-6 space-y-3 sm:space-y-4">
              {cartItems.map((item, index) => (
                <div key={index} className="bg-slate-900 border-2 border-slate-700 p-3 sm:p-4 hover:border-amber-600 transition-all">
                  <div className="flex gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-2">
                        <h3 
                          onClick={() => onProductClick(item)}
                          className="font-bold text-white text-sm sm:text-base leading-tight cursor-pointer hover:text-amber-400 transition-colors line-clamp-2"
                        >
                          {item.name}
                        </h3>
                        <button 
                          onClick={() => onRemoveFromCart(index)}
                          className="p-1 sm:p-1.5 text-amber-400 hover:text-amber-300 hover:bg-amber-900/20 transition-colors mr-1 sm:mr-2 flex-shrink-0"
                        >
                        ...
                        <div className="flex items-center gap-1 bg-slate-700 p-1">
                          <button 
                            ...
                            className={`w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center text-white font-bold ${
                              item.quantity > 1 ? 'bg-amber-600 hover:bg-amber-700' : 'bg-slate-600 cursor-not-allowed'
                            } transition-colors`}
                          >
                          ...
                          <button 
                            ...
                            className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center bg-amber-600 hover:bg-amber-700 text-white font-bold transition-colors"
                          >
        ...
        {cartItems.length > 0 && (
          <div className="border-t-2 border-amber-600 bg-black p-4 sm:p-6">
            {/* Order Summary */}
            ...
            <button 
              onClick={onCheckout}
              className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 px-6 transition-all border-2 border-amber-600 flex items-center justify-center gap-3"
            >
