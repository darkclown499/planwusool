  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" onClick={onClose}>
      ...
      <div className="absolute inset-0 flex items-end sm:items-center justify-center p-0 sm:p-2 md:p-4">
        <div className="bg-slate-800 w-full h-screen sm:h-auto sm:max-w-lg md:max-w-5xl sm:max-h-[95vh] md:max-h-[85vh] overflow-hidden flex flex-col border-2 border-amber-600 sm:rounded-lg" onClick={(e) => e.stopPropagation()}>
          
          {/* Header */}
          <div className="bg-slate-900 text-white p-3 sm:p-4 border-b-2 border-amber-600">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-6 h-6 sm:w-8 sm:h-8 bg-amber-600 flex items-center justify-center">
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                <WishlistButton productId={product.id} iconOnly className="p-1.5 sm:p-2 text-slate-300 hover:text-amber-400 hover:bg-slate-800 transition-colors" />
                <button onClick={onClose} className="p-1.5 sm:p-2 text-slate-300 hover:text-amber-400 hover:bg-slate-800 transition-colors">
          ...
          <div className="flex flex-col md:flex-row flex-1 md:overflow-hidden overflow-y-auto">
            {/* Image Gallery */}
            <div className="w-full md:w-1/2 bg-black border-l-0 md:border-l-4 border-amber-600 flex-shrink-0 overflow-y-auto">
                    ...
                    {product.images && product.images.length > 1 && (
                      <>
                        <button 
                          ...
                          className="absolute right-1 sm:right-2 top-1/2 -translate-y-1/2 bg-amber-600 hover:bg-amber-700 text-white p-1.5 sm:p-2 border border-white rtl-flip"
                        >
                        ...
                        <button 
                          ...
                          className="absolute left-1 sm:left-2 top-1/2 -translate-y-1/2 bg-amber-600 hover:bg-amber-700 text-white p-1.5 sm:p-2 border border-white rtl-flip"
                        >
                          <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" />
                          </svg>
                        </button>
                        <div className="absolute bottom-1 sm:bottom-2 left-1 sm:left-2 bg-amber-600 text-white px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-mono border border-white">
            ...
            <div className="flex-1 flex flex-col overflow-y-auto">
              <div className="flex-1 p-3 sm:p-4 md:p-6 bg-slate-800">
                ...
                <div className="bg-slate-900 border-4 border-slate-700 p-4">
                  <div className="border-r-4 border-amber-600 pr-4">
                    {product.category && (
                      <div className="bg-amber-600 text-white px-2 py-1 text-xs font-bold uppercase tracking-wider inline-block mb-3">
                        {product.category}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-black border-2 border-slate-700 p-3">
                      <div>
                        <div className="text-amber-400 text-xs font-bold uppercase">رقم القطعة</div>
                        <div className="text-white font-bold">{product.sku}</div>
                      </div>
                      <div>
                        <div className="text-amber-400 text-xs font-bold uppercase">الحالة</div>
                ...
                {/* Pricing */}
                <div className="bg-black border-4 border-amber-600">
                  ...
                  <div className="p-4">
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="bg-slate-900 border-2 border-slate-700 p-3">
                        <div className="text-amber-400 text-xs font-bold uppercase mb-1">سعر الوحدة</div>
                      ...
                      {product.originalPrice && (
                        <div className="bg-slate-900 border-2 border-slate-700 p-3">
                          <div className="text-amber-400 text-xs font-bold uppercase mb-1">السعر الأصلي</div>
                    ...
                    <div className="bg-slate-900 border-2 border-slate-700 p-3">
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <div className="text-amber-400 text-xs font-bold uppercase">التوفر</div>
                          <div className={`font-bold text-sm ${
                            product.availability === 'in_stock' ? 'text-green-400' : 'text-amber-400'
                          }`}>
                            {product.availability === 'in_stock' ? 'متوفر' : 'غير متوفر'}
                          </div>
                        </div>
                        <div>
                          <div className="text-amber-400 text-xs font-bold uppercase">الكمية</div>
                    ...
                    <div className="grid gap-4">
                      {product.variants.map((variant, index) => (
                        <div key={index}>
                          ...
                          >
                            <SelectTrigger className="w-full focus:ring-amber-500 focus:border-amber-500 border-slate-600 bg-slate-800 text-white">
                ...
                <div className="bg-slate-900 p-4 border-2 border-slate-700 sticky bottom-0">
                  ...
                  <WhatsAppOrderButton
                    ...
                    className="mt-3 w-full py-2 md:py-3 font-bold text-sm md:text-base transition-all flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1eb85a] text-white"
                  />
                </div>
                ...
                {product.customFields && product.customFields.length > 0 && (
                  <div className="bg-slate-900 p-4 md:p-6 border-2 border-amber-600">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                      <div className="w-1 h-6 bg-amber-600"></div>
                      المواصفات
                    </h3>
