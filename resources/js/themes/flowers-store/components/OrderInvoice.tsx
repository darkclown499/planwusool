    ...
    <>
      ...
      <div className="min-h-screen bg-red-50 py-4 md:py-8">
        <div className="max-w-4xl mx-auto px-4">
          {/* Header */}
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-4 md:mb-6 border-t-4 border-red-500">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 md:mb-6">
              <div className="mb-4 md:mb-0">
                <h1 className="text-xl md:text-2xl font-bold text-gray-900">فاتورة الطلب</h1>
                <p className="text-red-600 text-sm md:text-base font-medium">الطلب رقم #{orderNumber}</p>
              </div>
              <div className="flex gap-2 md:gap-3">
                ...
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white px-3 md:px-4 py-2 rounded-lg transition-colors cursor-pointer flex items-center gap-2 text-sm md:text-base print:hidden"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Order Info Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 bg-red-50 p-3 rounded-lg border border-red-100">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-red-700 font-semibold capitalize text-sm bg-red-100 px-2 py-1 rounded-full">{order.status}</span>
                </div>
              ...
              <div className="text-sm">
                <span className="text-gray-600">رقم الطلب:</span> <span className="font-semibold text-red-700">{order.id}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-semibold text-red-700 mb-3 text-base flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-4 md:mb-6">
            <h3 className="font-semibold text-red-700 mb-4 text-sm md:text-base flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b-2 border-red-100">
                    <th className="text-right py-2 md:py-3 px-2 md:px-4 font-semibold text-red-700 text-xs md:text-sm">المنتج</th>
                    <th className="text-center py-2 md:py-3 px-2 md:px-4 font-semibold text-red-700 text-xs md:text-sm">الكمية</th>
                    <th className="text-right py-2 md:py-3 px-2 md:px-4 font-semibold text-red-700 text-xs md:text-sm">السعر</th>
                    <th className="text-right py-2 md:py-3 px-2 md:px-4 font-semibold text-red-700 text-xs md:text-sm">الضريبة</th>
                    <th className="text-right py-2 md:py-3 px-2 md:px-4 font-semibold text-red-700 text-xs md:text-sm">الإجمالي</th>
                  </tr>
                </thead>
                <tbody>
                  {order.items.map((item, index) => {
                    ...
                    return (
                      <tr key={index} className="border-b border-red-50 hover:bg-red-50">
                        ...
                        <td className="py-3 md:py-4 px-2 md:px-4 text-right font-semibold text-red-700 text-xs md:text-sm">{formatCurrency(itemTotalWithTax, storeSettings, currencies)}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 border border-red-100">
            <div className="max-w-sm mr-auto">
              <h3 className="font-semibold text-red-700 mb-4 text-sm md:text-base">ملخص الطلب</h3>
              ...
              <div className="border-t-2 border-red-100 pt-2 md:pt-3">
                <div className="flex justify-between">
                  <span className="text-base md:text-lg font-semibold text-gray-900">الإجمالي</span>
                  <span className="text-lg md:text-xl font-bold text-red-600">{formatCurrency(order.total, storeSettings, currencies)}</span>
                </div>
              </div>
          ...
          <div className="text-center mt-8 text-gray-500 text-sm bg-white p-6 rounded-lg">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="font-medium text-red-600">شكراً لاختيارك متجرنا!</span>
            </div>
