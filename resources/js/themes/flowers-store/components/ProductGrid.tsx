      ...
      {products.map((product) => (
        ...
        >
          ...
          <div className="relative aspect-[3/4] md:aspect-square overflow-hidden bg-gray-50">
            ...
            <div className="absolute top-3 left-3 flex flex-col space-y-2">
              ...
              {product.originalPrice && (
                <div className="w-fit bg-red-500 text-white text-xs font-medium px-2 py-1 rounded-full">
            ...
            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col space-y-2">
              ...
              <button 
                onClick={() => onProductClick(product)}
                className="w-8 h-8 bg-white rounded-lg shadow-md flex items-center justify-center text-gray-600 hover:text-red-500 transition-colors"
              >
            ...
            <div className="absolute bottom-3 left-3 right-3 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors duration-300 shadow-lg"
              >
                {product.availability === 'out_of_stock' 
                  ? 'غير متوفر' 
                  : (product.variants && Array.isArray(product.variants) && product.variants.length > 0)
          ...
          <div className="p-3 md:p-4">
            ...
            <h3 
              onClick={() => onProductClick(product)}
              className="font-medium text-gray-900 mb-2 line-clamp-2 cursor-pointer hover:text-red-600 transition-colors text-sm md:text-base"
            >
              {product.name}
            </h3>
              ...
              <div className="mb-3">
                <div className="flex flex-wrap gap-1">
                  {product.variants.slice(0, 3).map((variant, index) => (
                    <span
                      key={index}
                      className="text-xs px-2 py-1 bg-red-100 text-red-600 rounded border hover:border-red-300 cursor-pointer transition-colors"
                    >
                      {variant.name}
                    </span>
                  ))}
