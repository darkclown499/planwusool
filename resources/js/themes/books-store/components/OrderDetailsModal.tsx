      ...
      <div className="fixed inset-0 z-60 overflow-hidden" onClick={onClose}>
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <p className="text-amber-600 font-serif">جارٍ تحميل تفاصيل الطلب...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-60 overflow-hidden" onClick={onClose}>
      ...
      <div className="absolute inset-0 flex items-center justify-center p-2 sm:p-4">
        <div className="bg-amber-50 w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] rounded-t-2xl sm:rounded-2xl shadow-2xl border border-amber-200 flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="relative bg-amber-700 p-4 sm:p-5 flex-shrink-0">
            ...
            <div className="text-center text-white">
              ...
              <p className="text-amber-200 text-xs sm:text-sm">الطلب رقم #{orderNumber}</p>
            </div>
          </div>

          {/* Content */}
          <div className="flex-1 overflow-y-auto bg-amber-50 min-h-0">
            {/* Hero Card */}
            <div className="bg-amber-50 rounded-3xl p-6 m-4 border-2 border-amber-200 shadow-lg">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-amber-700 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0">
                    <Package className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-xl sm:text-2xl font-serif font-bold text-amber-900">#{order.id}</h3>
                ...
                <div className="text-right">
                  <div className="text-2xl sm:text-3xl font-serif font-bold text-amber-900 mb-2">{formatCurrency(order.total, storeSettings, currencies)}</div>
            ...
            <div className="px-4 pb-4 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white rounded-2xl p-5 shadow-md border border-amber-200">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 bg-amber-700 rounded-xl flex items-center justify-center">
                      <Receipt className="w-5 h-5 text-white" />
                    </div>
                    <h4 className="font-serif font-bold text-amber-900">العميل</h4>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs text-amber-500 uppercase tracking-wide">الاسم</label>
                      <p className="font-semibold text-amber-900">{order.customer.name}</p>
                    </div>
                    <div>
                      <label className="text-xs text-amber-500 uppercase tracking-wide">البريد الإلكتروني</label>
                      <p className="text-amber-600 text-sm">{order.customer.email}</p>
                    </div>
                    <div>
                      <label className="text-xs text-amber-500 uppercase tracking-wide">الهاتف</label>
                      <p className="text-amber-600 text-sm">{order.customer.phone}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-md border border-amber-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h4 className="font-serif font-bold text-amber-900">عنوان الشحن</h4>
                  </div>
                  <div className="space-y-2">
                    <div>
                      <label className="text-xs text-amber-500 uppercase tracking-wide">المستلم</label>
                      <p className="font-semibold text-amber-900">{order.shipping_address.name}</p>
                    </div>
                    <div>
                      <label className="text-xs text-amber-500 uppercase tracking-wide">العنوان</label>
              ...
              <div className="bg-white rounded-2xl shadow-md border border-amber-200 overflow-hidden">
                <div className="bg-amber-50 px-6 py-4 border-b border-amber-200">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="font-serif font-bold text-amber-900">منتجات الطلب ({order.items.length})</h4>
                  </div>
                </div>
                ...
                <div className="divide-y divide-amber-100">
                  {order.items.map((item: any, index: any) => {
                    ...
                    return (
                      <div key={index} className="p-5 hover:bg-amber-50 transition-colors">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          <div className="flex-1">
                            <h5 className="font-serif font-semibold text-amber-900 mb-1">{item.name}</h5>
                            {variants && Object.keys(variants).length > 0 && (
                              <div className="flex flex-wrap gap-1 mb-2">
                                {Object.entries(variants).map(([key, value], idx) => (
                                  <span key={key} className="bg-amber-200 px-2 py-1 rounded-lg text-xs text-amber-700">
                                    {key}: {value}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex items-center gap-4 text-sm text-amber-600">
                              <span className="bg-amber-100 px-2 py-1 rounded">الكمية: {item.quantity}</span>
                          ...
                          <div className="text-right">
                            ...
                            {item.tax_amount > 0 && (
                              <div className="text-xs text-amber-500">شامل الضريبة</div>
                            )}
                          </div>
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-2xl shadow-md border border-amber-200 overflow-hidden">
                <div className="bg-amber-50 px-6 py-4 border-b border-amber-200">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="font-serif font-bold text-amber-900">ملخص الطلب</h4>
                  </div>
                </div>
          ...
          {/* Footer */}
          <div className="border-t border-amber-200 p-4 sm:p-6 flex-shrink-0 bg-amber-50">
            <div className="flex justify-end">
              <button
                ...
                className="bg-amber-700 hover:bg-amber-800 text-white px-6 py-3 rounded-xl font-serif font-semibold transition-colors flex items-center gap-2 shadow-lg"
              >
