...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-amber-50 border-b-2 border-amber-300 shadow-sm">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex flex-1 max-w-md mx-8">
              <div className="w-full relative">
                <input
                  ...
                  className="w-full pl-4 pr-12 py-2 border-2 border-amber-300 focus:border-amber-500 focus:outline-none rounded-lg bg-white text-amber-900 placeholder-amber-600"
                />
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              <button
                onClick={onCartClick}
                className="relative p-3 text-amber-700 hover:text-amber-900 hover:bg-amber-100 rounded-full transition-colors cursor-pointer"
              >
              ...
              {isLoggedIn ? (
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 p-2 text-amber-700 hover:text-amber-900 hover:bg-amber-100 rounded-lg transition-colors cursor-pointer"
                  >
                  ...
                  {showUserMenu && (
                    <div className="absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-amber-300 py-2 z-50">
                      <button
                        onClick={() => {
                          onProfileClick();
                          setShowUserMenu(false);
                        }}
                        className="w-full text-right px-4 py-2 text-amber-800 hover:bg-amber-50 transition-colors cursor-pointer"
                      >
                        ملفي الشخصي
                      </button>
                      <button
      ...
      {showMobileSearch && (
        <div className="md:hidden fixed top-16 left-0 right-0 z-50 bg-amber-50 border-b-2 border-amber-300 px-4 py-3">
          <div className="relative">
            <input
              type="text"
              placeholder="ابحث عن المنتجات..."
              value={searchQuery}
              onChange={handleSearchChange}
              className="w-full pl-4 pr-12 py-3 border-2 border-amber-300 focus:border-amber-500 focus:outline-none rounded-lg bg-white text-amber-900 placeholder-amber-600"
              autoFocus
            />
            <button
              onClick={toggleMobileSearch}
      ...
      {showUserMenu && isLoggedIn && (
        <div className="md:hidden fixed top-16 left-0 right-0 z-50 bg-white border-b-2 border-amber-300 py-4">
          <div className="px-4 space-y-2">
            <button
              ...
              className="w-full text-right px-4 py-3 text-amber-800 hover:bg-amber-50 rounded-lg transition-colors cursor-pointer"
            >
              ملفي الشخصي
            </button>
            <button
